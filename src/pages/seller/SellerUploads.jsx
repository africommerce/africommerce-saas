...
import { Box, Typography, Button, Checkbox, TextField, Select, MenuItem, Card, CardMedia} from '@mui/material';
...
export const SellerUploads = () => {
  ...
  return (
    <Box sx={{pr: 4, backgroundColor: 'white', p: '20px'}}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'row', border: 1, borderBottom: 0, py: 3, px: 3, justifyContent: "space-between" }}>
      ...
      All Files
        ...
        <Select labelId="label" id="select" value="10" size="small" sx={{mx: 1.5, pr: 15}}>
          ...
          {images.map(({ id, src}) => {
            return (
              <Card key={id} sx={{border: '1px solid grey', p: '20px'}}>
                ...
                <CardMedia component="img" image={src} alt= "vv" sx={{ borderRadius: 2, align: 'center', }}/>
                </Box>
              </Card>
            )
          })}
